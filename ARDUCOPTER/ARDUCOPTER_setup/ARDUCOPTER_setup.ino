...
void loop(){
  ...
  if(error == 0){
    ...
    Serial.println(F("You can now calibrate the esc's and upload the esc callibration code."));
  }
  else{
   Serial.println(F("The setup is aborted due to an error."));
   Serial.println(F("Check the ReadMe page of this project on:"));
   Serial.println(F("https://github.com/i-akb25/ARDUCOPTER for more information about this error."));
  }
  while(1);
}
...
void intro(){
  ...
  delay(1000);
  Serial.println(F(""));
  Serial.println(F("ARDUCOPTER Setup Program"));
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  delay(1500);
  Serial.println(F("For support and questions: mailto:anuragaryanofficial@gmail.com"));
  Serial.println(F(""));
